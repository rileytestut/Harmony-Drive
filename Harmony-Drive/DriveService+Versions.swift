    ...
    public func fetchVersions(for record: RemoteRecord, completionHandler: @escaping (Result<[Version]>) -> Void) -> Progress
    {
        ...
        let ticket = self.service.executeQuery(query) { (ticket, object, error) in
            guard error == nil else {
                ...
                {
                    return completionHandler(.failure(FetchVersionsError(record: managedRecord, code: .fileDoesNotExist)))
                }
                else
                {
                    return completionHandler(.failure(FetchVersionsError(record: managedRecord, code: .any(error!))))
                }
            }
            
            guard let revisionList = object as? GTLRDrive_RevisionList, let revisions = revisionList.revisions else {
                return completionHandler(.failure(FetchVersionsError(record: managedRecord, code: .invalidResponse)))
            }
        ...
        progress.cancellationHandler = {
            ticket.cancel()
            completionHandler(.failure(FetchVersionsError(record: managedRecord, code: .cancelled)))
        }
